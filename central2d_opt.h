#ifndef CENTRAL2D_OPT_H
#define CENTRAL2D_OPT_H
...
// Opt
////////////////////////////////////////////////////////////////////////////////
// Opt is a friend of Central2DOpt and Opt objects take in Central2DOpt
// objects, so we have a cyclic dependency. We break it with a forward
// declaration of Central2DOpt.
...
public:
    ...
    Opt(Central2DOpt<Physics, Limiter> *parent, const int ix, const int iy,
...
private:
    Central2DOpt<Physics, Limiter> *parent_;
    const int ix_;
    const int iy_;
    const int nx_;
    const int ny_;
...
template <class Physics, class Limiter>
void Opt<Physics, Limiter>::limited_derivs() {
    using L = Limiter;
...
template <class Physics, class Limiter>
void Opt<Physics, Limiter>::compute_step() {
...
public:
    ...
    Central2DOpt(real w, real h,     // Domain width / height
    ...
    Central2DOpt(const Central2DOpt&) = delete;
...
private:
    friend class Opt<Physics, Limiter>;
...
template <class Physics, class Limiter>
template <typename F>
void Central2DOpt<Physics, Limiter>::init(F f) {
...
template <class Physics, class Limiter>
void Central2DOpt<Physics, Limiter>::apply_periodic() {
...
template <class Physics, class Limiter>
void Central2DOpt<Physics, Limiter>::compute_max_speed(real& cx, real& cy) {
    using namespace std;
    real _cx = 1.0e-15;
    real _cy = 1.0e-15;
...
template <class Physics, class Limiter>
void Central2DOpt<Physics, Limiter>::run_block(const int io,
                                                 const real dt,
                                                 const int bx,
                                                 const int by) {
    //
    ...
    // step block
    Opt<Physics, Limiter> b(this, ix, iy, _u, width, height, bghosts, dx, dy, io, dt);
    b.step();
}


template <class Physics, class Limiter>
void Central2DOpt<Physics, Limiter>::run(real tfinal)
{
...
template <class Physics, class Limiter>
void Central2DOpt<Physics, Limiter>::solution_check()
{
    using namespace std;
