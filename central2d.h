...
void Central2D<Physics, Limiter>::run(real tfinal)
{
    ...
    while (!done) {
        ...
        #pragma omp parallel num_threads(numProcs) 
        {
            ...
            int offset_local(int ix, int iy) const { return iy*nx_all_local+ix; }

            vec& u_l(int ix, int iy)    { return u_local[offset_local(ix,iy)]; }
            vec& v_l(int ix, int iy)    { return v_local[offset_local(ix,iy)]; }
            vec& f_l(int ix, int iy)    { return f_local[offset_local(ix,iy)]; }
            vec& g_l(int ix, int iy)    { return g_local[offset_local(ix,iy)]; }

            vec& ux_l(int ix, int iy)   { return ux_local[offset_local(ix,iy)]; }
            vec& uy_l(int ix, int iy)   { return uy_local[offset_local(ix,iy)]; }
            vec& fx_l(int ix, int iy)   { return fx_local[offset_local(ix,iy)]; }
            vec& gy_l(int ix, int iy)   { return gy_local[offset_local(ix,iy)]; }
