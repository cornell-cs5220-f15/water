#include "central2dvecpoint.h"
#include "shallow2dvecpoint.h"
#include "minmodvecpoint.h"
#include "meshiovecpoint.h"
...
void dam_break(Sim::real* u, double x, double y, int stride)
{
    x -= 1;
    y -= 1;
...
void pond(Sim::real* u, double x, double y, int stride)
{
    *u=1.0;
    *(u+stride)=0;
    *(u+2*stride)=0;
...
void river(Sim::real* u, double x, double y, int stride)
{
    *u=1.0;
    *(u+stride)=1.0;
    *(u+2*stride)=0;
...
void wave(Sim::real* u, double x, double y, int stride)
{
    using namespace std;
...
int main(int argc, char** argv)
{
    ...
    void (*icfun)(Sim::real* u, double x, double y,int stride) = dam_break;
    if (ic == "dam_break") {
        icfun = dam_break;
    } else if (ic == "pond") {
        icfun = pond;
