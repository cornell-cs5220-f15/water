    ...
    public:
        ...
        {   for (int i=0; i<nblocks; ++i) {
                ...
                for (int j=0; j<nblocks; ++j)
                    blocks[i].push_back(SimBlock(blockWidth, blockHeight,
                            nx_block, ny_block, cfl));
            }
        }
...
void BlockedSimulation::copy_ghosts() {
    for (int i=0; i<nblocks; i++)
        for (int j=0; j<nblocks; j++) {
            ...
            self.copy_ghosts_form_left(blocks[i][j_left]);
            self.copy_ghosts_form_topleft(blocks[i_top][j_left]);
            self.copy_ghosts_form_bottomleft(blocks[i_bot][j_left]);
            self.copy_ghosts_form_right(blocks[i][j_right]);
            self.copy_ghosts_form_topright(blocks[i_top][j_right]);
