...
public:
    ...
    BlockPar(real *u, int nx, int ny, int nghost, real dx, real dy, int io, real dt) :
        nx_(nx),
        ny_(ny),
        nghost_(nghost),
        dx_(dx),
    ...
    BlockPar(const BlockPar&) = delete;
    BlockPar(BlockPar&&)      = delete;
...
template <class Physics, class Limiter>
void BlockPar<Physics, Limiter>::flux() {
...
template <class Physics, class Limiter>
void BlockPar<Physics, Limiter>::limited_derivs() {
    using L = Limiter;
...
template <class Physics, class Limiter>
void BlockPar<Physics, Limiter>::compute_step() {
...
template <class Physics, class Limiter>
void BlockPar<Physics, Limiter>::step() {
    flux();
    limited_derivs();
    compute_step();
}
...
public:
    ...
    Central2DBlockPar(const Central2DBlockPar&) = delete;
...
void Central2DBlockPar<Physics, Limiter>::run_block(const int io,
    ...
    // step block
    BlockPar<Physics, Limiter> b(_u, width, height, bghosts, dx, dy, io, dt);
    b.step();
