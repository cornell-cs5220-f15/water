...
static
central2d_t* createRectangle(float w, float h, int nx, int ny,
                             int nfield, flux_t flux, speed_t speed,
                             float cfl,int timef)
{
    int ng = 3*timef;
...
static
void copytooriginal(central2d_t* region,central2d_t* sim,int processor, int nthreads)
{
...
int central2d_xrun(float* restrict u, float* restrict v,
                   ...
                   int nfield, flux_t flux, speed_t speed, central2d_t* sim,
                   float tfinal, float dx, float dy, float cfl, int nthreads, int timef)
{
    int nstep = 0;
    int nx_all = nx + 2*ng;
    int ny_all = ny + 2*ng;
    ...
    while (!done) {
        #pragma omp single
      {
        ...
        for (iter=0;iter<timef;++iter){
            central2d_step(region->u, region->v, region->ux, region->uy, region->f,
                           region->fx, region->g, region->gy,
...
int central2d_run(central2d_t* sim, float tfinal)
{
   ...
   return central2d_xrun(sim->u, sim->v, sim->ux, sim->uy,
                         ...
                         tfinal, sim->dx, sim->dy, sim->cfl,nthreads);
}
