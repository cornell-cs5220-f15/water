#ifndef CENTRAL2D_par_H
#define CENTRAL2D_par_H
...
public:
    ...
    Central2DPar(real w, real h,     // Domain width / height
...
template <class Physics, class Limiter>
template <typename F>
void Central2DPar<Physics, Limiter>::init(F f)
{
...
template <class Physics, class Limiter>
void Central2DPar<Physics, Limiter>::compute_fg_speeds(real& cx_, real& cy_)
{
    using namespace std;
    real cx = 1.0e-15;
    real cy = 1.0e-15;
...
template <class Physics, class Limiter>
void Central2DPar<Physics, Limiter>::compute_step(int io, real dt)
{
...
template <class Physics, class Limiter>
void Central2DPar<Physics, Limiter>::run(real tfinal)
{
    bool done = false;
    real t = 0;
    while (!done) {
...
template <class Physics, class Limiter>
void Central2DPar<Physics, Limiter>::solution_check()
{
    using namespace std;
