...
private:
 ...
 /*   static void limdiff(vec& du, const vec& um, const vec& u0, const vec& up) {
    ...
    void apply_periodic(real* u,
                        int nx, int ny, int ng, int nfield);
    void compute_fg_speeds(real& cx, real& cy);
    void limited_derivs();
    void compute_step(int io, real dt);
    void copy_subgrid(real* dst, real* src,
                      int nx, int ny, int stride);
    void limited_deriv1(real* du,const real* u,int ncell);
    void limited_derivk(real* du,const real* u,int ncell, int stride);
    void limited_derivs(real* ux, real* uy,
                        real* fx, real* gy,
                        const real* u,
                        const real* f,
                        const real* g,
                        int nx, int ny, int nfield);
    void central2d_predict(real* v,
                           const real* u,
                           const real* fx,
                           const real* gy,
                           float dtcdx2, float dtcdy2,
                           int nx, int ny, int nfield);
    void central2d_correct(real* v,
                           const real* u,
                           const real* ux,
                           const real* uy,
                           const real* f,
                           ...
                           int nx, int ny, int nfield);
    void compute_step(real* u, real* v,
                      real* ux, real* uy,
                      real* f, real* fx,
                      real* g, real* gy,
 ...
 */
template <class Physics, class Limiter>
void Central2D<Physics, Limiter>::copy_subgrid(real*  dst,
                  real*  src,
                  int nx, int ny, int stride)
{
    for (int iy = 0; iy < ny; ++iy)
...
template <class Physics, class Limiter>
void Central2D<Physics, Limiter>::apply_periodic(real*  u,
                                                 int nx, int ny, int ng, int nfield)
{
    int s = nx + 2*ng;
...
template <class Physics, class Limiter>
void Central2D<Physics, Limiter>::limited_deriv1(real*  du,const real* u,int ncell)
{
...
template <class Physics, class Limiter>
void Central2D<Physics, Limiter>::limited_derivk(real*  du,const real* u,int ncell, int stride)
{
    assert(stride>0);
    for (int i=0; i<ncell; ++i)
...
template <class Physics, class Limiter>
void Central2D<Physics, Limiter>::limited_derivs(real*  ux, real*  uy,
                                                 real*  fx, real*  gy,
                                                 const real*  u,
                                                 const real*  f,
                                                 const real*  g,
 ...
 */
template <class Physics, class Limiter>
void Central2D<Physics, Limiter>::central2d_predict(real* v,
                       const real* u,
                       const real*  fx,
                       const real*  gy,
                       float dtcdx2, float dtcdy2,
...
template <class Physics, class Limiter>
void Central2D<Physics, Limiter>::compute_step(real*  u, real*  v,
                                               real*  ux, real*  uy,
                                               real*  f, real*  fx,
                                               real*  g, real*  gy,
                                               int io, int nx, int ny, int ng,
...
template <class Physics, class Limiter>
int Central2D<Physics, Limiter>::xrun(real*  u, real*  v,
                                       real*  ux, real*  uy,
                                       real*  f, real*  fx,
                                       real*  g, real*  gy,
                                       int nx, int ny, int ng,
