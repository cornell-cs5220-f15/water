...
struct Shallow2D {
    ...
    static void flux(vec& FU, vec& GU, const vec& U, int nx_inf,int nx_sup, int ny_inf, int ny_sup, int nx_all) {
	    for (int iy = ny_inf; iy < ny_sup; ++iy){
	       	for (int ix = nx_inf; ix < nx_sup; ++ix){
				int ind = 3*(iy*nx_all+ix);
				real h = U[ind], hu = U[ind+1], hv = U[ind+2];

        		FU[ind] = hu;
        		FU[ind+1] = hu*hu/h + (0.5*g)*h*h;
        		FU[ind+2] = hu*hv/h;

        		GU[ind] = hv;
        		GU[ind+1] = hu*hv/h;
        		GU[ind+2] = hv*hv/h + (0.5*g)*h*h;
			}
	  	}
    ...
    static void wave_speed(real& cx, real& cy, const vec& U, int nx_inf,int nx_sup, int ny_inf, int ny_sup, int nx_all) {
        using namespace std;
        real cx2,cy2
	for (int iy = ny_inf; iy < ny_sup; ++iy){
	    for (int ix = nx_inf; ix < nx_sup; ++ix){
		int ind = 3*(iy*nx_all+ix);
        	...
        	cx2 = fabs(hu/h) + root_gh;
        	cy2 = fabs(hv/h) + root_gh;
		cx = fmax(cx,cx2);
		cy = fmax(cy,cy2);
	    }
	}
